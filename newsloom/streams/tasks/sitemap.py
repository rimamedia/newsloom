...
class BaseSitemapTask(luigi.Task):
    """Base class for sitemap parsing tasks"""
    ...
    max_links = luigi.IntParameter(default=100, description="Maximum number of links to process")
    scheduled_time = luigi.Parameter(description="Scheduled execution time")
    follow_next = luigi.BoolParameter(default=False, description="Whether to follow next page links in sitemaps")
    ...
    def output(self):
        return luigi.LocalTarget(f'/tmp/sitemap_task_{self.stream_id}_{datetime.now().strftime("%Y%m%d_%H%M%S")}')

    def process_url(self, stream, url, lastmod):
        """Process individual URL - to be implemented by subclasses"""
        raise NotImplementedError


class SitemapNewsParsingTask(BaseSitemapTask):
    """Task for parsing news sitemaps"""
    
    def process_url(self, stream, url, lastmod):
        published_at = None
        if lastmod is not None:
            try:
                published_at = datetime.fromisoformat(lastmod.text.strip().replace('Z', '+00:00'))
            except ValueError:
                pass
...
class SitemapBlogParsingTask(BaseSitemapTask):
    """Task for parsing blog sitemaps"""
