...
from .playwright import USER_AGENTS, save_links, update_stream_status
...
def search_articles(
    ...
    if not stream.source:
        raise Exception("Stream has no source")
                ...
                try:
                    ...
                    if article_content:
                        ...
                        if search_text.lower() in content_text:
                            ...
                            logger.info(f"Found matching article: {link_data['url']}")
                except Exception as e:
    ...
    # Save matching links
    result["saved_count"] = create_news_from_links(matching_links, stream.source)

    logger.info(f"Successfully processed {result['extracted_count']} links, "
